...
#       fi
        ...
        findMeanSD.py  -R ${basename}".report" > ${basename_c}".mean.sd.txt"


        #2) Regression coefficients for the mean and stdev
        echo "2) Calculating regression coefficients for the mean and stdev "
        Rscript `which findBetas.r` ${basename_c}".mean.sd.txt" ${basename_c}".betas.txt" 1
        ...
        findThresholds.py -B ${basename_c}".betas.txt" -R ${basename}".report" -Z ${Z} -I ${I} > ${basename_c}".output.thresholds.txt"
        ...
        calibrateZ.py -R ${basename}".report" -T ${basename_c}".output.thresholds.txt" > ${basename_c}".output.thresholds.txt".stats
